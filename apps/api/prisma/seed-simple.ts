...
async function main() {
  ...
  if (!demoUser) {
    console.log('❌ 请先运行基础种子脚本（npm run seed）');
    return;
  }
  ...
  // 3. 创建候选人（使用 Prisma 的 create 方法，不使用 upsert 以避免冲突）
  console.log('\n3️⃣  创建候选人...');
    ...
    data: [
      {
        ...
        source: 'DIRECT',
        transparencyLevel: 'STANDARD' as any,
      },
      {
        id: 'test-app-002',
  ...
  const interviews = await prisma.interview.createMany({
    data: [
      ...
      },
      {
        ...
        title: 'UI设计作品展示',
        type: InterviewType.VIDEO,
        stage: InterviewStage.PORTFOLIO,
